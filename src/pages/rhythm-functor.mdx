...
The `identity` function I discussed in the last post is a generalization of this, and polymorphically applies to _all_ types in 𝕻.
...
The parentheses around `f a -> f b` can be dropped and it would mean the same thing, but I like them because they make it clear that we are mapping a morphism `a -> b` to a _new_ moprhism from `f a -> f b`, where both `f a` and `f b` are objects somewhere else in the vast category of 𝕻.
...
Somewhat-formally speaking, a pro-functor takes a pair of morphisms - one from an opposite (contravariant) category and one from a "normal" unflipped catgory, and maps them to yet-another category. As all we have is 𝕻, we'll settle for (𝕻op x 𝕻) being lifted to another region of 𝕻, where 𝕻op is 𝕻 flipped. Note that this flip sort of makes sense - for most functions (ie `Int -> Boolean`) we can conceive of a function in the oppostie direction (`Boolean -> Int`). This falls apart, however, for functions with un
