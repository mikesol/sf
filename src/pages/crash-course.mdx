...
Wags is written in [PureScript](https://purescript.org). But you don't need to know PureScript in order to use wags. In the example above, change `hh:0` to `bd:0` (bass drum), stop it, and start it again. Change it to `sn:0` (snare), stop it, and start it again. You can change to any of the sounds from the [Amazing SuperDirt Sample Library](https://github.com/mikesol/wagsi/blob/main/SOUNDS.md) and you should hear something.

You can copy this and all other examples into the _blank slate_ track on [wags.fm](https://wags.fm) and they will work out of the box.

## More notes

When you add more events in the sequence, they are played in sequence.  The example above has four different events. You can add some or subtract some, stop, and play again to hear the difference.

<Player player={severalMain} code={several} />

## Faster loops

Let's make our loop faster. You'll see that this example is the same as the example above but twice the speed. This is because the first value passed to `make` is `0.4`, or half of the value used above.
...
Sometimes, you want two events to happen at the same time. You can do this by putting square brackets (`[` and `]`) around them and separating them with a comma. More than two events? No problem, just add more commas.
...
Hearing the same loop over and over again is boring. Spice it up with _branching_. Each time the example loops, a different branch will be chosen.

> Branches multiply, so something that branches two times, three times and then four times will be 2*3*4 cycles long.

<Player player={branchMain} code={branch} />

## Polyrhythms

You can have more than one note when you use a comma (`,`) between square brackets. When you do this, you create polyrhythms. Let's listen to 3 on 4.

<Player player={polyMain} code={poly} />

## Voices

Up until now, we've only used one voice called `earth`. Let's add a new voice called `wind`. The three possible voices are `earth`, `wind`, and `fire`. Different voices are helpful when you're doing a jam session, as different DJs can work on different voices. They're also useful if you want to add effects (we'll add effects later).
...
You can give any note a tag using a semi-colon (`;`). Then, use the `onTag` function to grab the note and `changeVolume` to change the volume. Note the use of `const`, which is a PureScript function that returns constant values.

<Player stub player={singleMain} code={single} />
...
- `clockTime` to change the rate
- `sampleTime` to change the volume
...
You can add aribtrary effects chains to voices.

### High-pass filter

Let's add a high-pass filter to create a sweeping effect.
...
To create a feedback delay, use the `delay` effect as well as `ref`, which is a reference to another node in the graph. In this case, `ref` refers to `myGain`, which is the gain node that we created elsewhere in the graph.

<Player stub player={singleMain} code={single} />

### Other effects

I unfortunately haven't had time to document all the effects yet, but they are all here in [this module](https://github.com/mikesol/purescript-wags/blob/main/src/WAGS/Create/Optionals.purs). Here is a "kitchen sink" example that uses a lot of them.
...
- dive into the examples on [`purescript-wags-lib`](https://github.com/mikesol/purescript-wags-lib) for more advanced use cases, including using [TensorFlow.js to create markov models]() and [controlling the music with a mouse]().
